...
// POST /api/score — Whisper 採点（base64音声 → 転写 → スコア）
app.post('/api/score', async (req, res) => {
  const { audio, mimeType = 'audio/webm', reference } = req.body;
  if (!audio || !reference) return res.status(400).json({ error: 'audio と reference が必要です' });
  ...
  try {
    ...
    const score      = calcScoreServer(reference, transcript);
    console.log(`[score] ref="${reference.slice(0, 40)}…" transcript="${transcript.slice(0, 60)}" score=${score}`);
...
function calcScoreServer(reference, transcript) {
